...
WINDOW *dupwin(WINDOW *win)
{
    ...
    if ( !(_new = PDC_makenew(nlines, ncols, begy, begx))
        || !(_new = PDC_makelines(_new)) )
        return (WINDOW *)NULL;
    ...
    for (i = 0; i < nlines; i++)
    {
        for (ptr = _new->_y[i], ptr1 = win->_y[i];
             ptr < _new->_y[i] + ncols; ptr++, ptr1++)
            *ptr = *ptr1;

        _new->_firstch[i] = 0;
        _new->_lastch[i] = ncols - 1;
    }

    _new->_curx = win->_curx;
    _new->_cury = win->_cury;
    _new->_maxy = win->_maxy;
    _new->_maxx = win->_maxx;
    _new->_begy = win->_begy;
    _new->_begx = win->_begx;
    _new->_flags = win->_flags;
    _new->_attrs = win->_attrs;
    _new->_clear = win->_clear;
    _new->_leaveit = win->_leaveit;
    _new->_scroll = win->_scroll;
    _new->_nodelay = win->_nodelay;
    _new->_use_keypad = win->_use_keypad;
    _new->_tmarg = win->_tmarg;
    _new->_bmarg = win->_bmarg;
    _new->_parx = win->_parx;
    _new->_pary = win->_pary;
    _new->_parent = win->_parent;
    _new->_bkgd = win->_bkgd;
    _new->_flags = win->_flags;
...
WINDOW *resize_window(WINDOW *win, int nlines, int ncols)
{
    ...
    {
        if ( !(_new = subpad(win->_parent, nlines, ncols,
                            win->_begy, win->_begx)) )
            return (WINDOW *)NULL;
    }
    else if (win->_flags & _SUBWIN)
    {
        if ( !(_new = subwin(win->_parent, nlines, ncols,
                            win->_begy, win->_begx)) )
            return (WINDOW *)NULL;
    }
    else
    {
        ...
        if ( !(_new = PDC_makenew(nlines, ncols, new_begy, new_begx)) )
            return (WINDOW *)NULL;
    }

    save_curx = min(win->_curx, _new->_maxx);
    save_cury = min(win->_cury, _new->_maxy);

    if (!(win->_flags & (_SUBPAD|_SUBWIN)))
    {
        if ( !(_new = PDC_makelines(_new)) )
            return (WINDOW *)NULL;

        werase(_new);

        copywin(win, _new, 0, 0, 0, 0, min(win->_maxy, _new->_maxy) - 1,
                min(win->_maxx, _new->_maxx) - 1, FALSE);
    ...
    _new->_nodelay = win->_nodelay;
    _new->_use_keypad = win->_use_keypad;
    _new->_tmarg = (win->_tmarg > _new->_maxy - 1) ? 0 : win->_tmarg;
    _new->_bmarg = (win->_bmarg == win->_maxy - 1) ?
                  _new->_maxy - 1 : min(win->_bmarg, (_new->_maxy - 1));
    _new->_parent = win->_parent;
    _new->_immed = win->_immed;
    _new->_sync = win->_sync;
    _new->_bkgd = win->_bkgd;

    _new->_curx = save_curx;
    _new->_cury = save_cury;
